...
description: Creates specs before coding. Use when starting a new project, feature, or significant change and no specification exists yet. Use when drafting a PRD or requirements document with objectives and scope, or when requirements are unclear, ambiguous, or only exist as a vague idea. Use when a single requirement spans several independently testable capabilities and needs decomposing into a capability map of modules before specifying. Triggered by feature requests or vague product tasks.
license: Complete terms in LICENSE.txt
---

<!--
Sourced from `spec-driven-development` in https://github.com/addyosmani/agent-skills
(MIT License), renamed to `requirements-analysis` to match this project's SDLC
skill catalog. Cross-references to sibling skills from the source repo that
are included here (`implementation-planning`, `testing-and-verification`)
were renamed to match; references to skills or `../../references/*.md` files
not included in this catalog are called out inline instead of left as broken
paths.
-->
...
- **Interfaces live at the boundary.** The map records that `billing` depends on `identity`; the contract between them belongs in the provider module's spec (an API/interface-design skill, if this catalog has one, is the natural place to design it).
...
> Follow the `implementation-planning` skill for the dependency-graph mapping and vertical-slicing mechanics behind these steps; it is the canonical source. The bullets above are a lightweight summary; if they ever diverge, `implementation-planning` takes precedence.
>
> **Output convention:** Save the plan to `tasks/plan.md` and record the task list in the task list target defined by `implementation-planning` (default `tasks/todo.md`; projects may designate an external tracker instead). Create `tasks/` if it does not exist.
...
> Follow the `implementation-planning` skill for the full task-sizing and dependency-ordering mechanics; it is the canonical source. The template below is a lightweight inline form; if they ever diverge, `implementation-planning` takes precedence.
...
Execute tasks one at a time, applying the `testing-and-verification` skill as each task is built (test-driven where the codebase supports it). Load only the spec sections and source files relevant to the task at hand rather than flooding the agent with the entire spec.
