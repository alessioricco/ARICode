...
description: Drives development with tests using the red-green-refactor loop. Use when implementing any logic, fixing any bug, or changing any behavior. Use when you need to prove that code works, when a bug report arrives, or when you're about to modify existing functionality. Triggered by implementation tasks.
license: Complete terms in LICENSE.txt
---

<!--
Sourced from `test-driven-development` in https://github.com/addyosmani/agent-skills
(MIT License), renamed to `testing-and-verification` to match this project's
SDLC skill catalog. The pointer to the sibling `browser-testing-with-devtools`
skill and to a `../../references/testing-patterns.md` file — neither included
in this catalog — were adjusted to say so rather than left as broken paths.
-->
...
**Related:** For browser-based changes, combine TDD with runtime verification (Chrome DevTools or equivalent) — see the Browser Testing section below.
...
For anything that runs in a browser, unit tests alone aren't enough — you need runtime verification: DOM inspection, console logs, network requests, performance traces, and screenshots (Chrome DevTools, or an equivalent runtime inspection tool, gives an agent this kind of access — not included as a separate skill in this catalog).
